---
title: ".NETにおけるStrategyパターン実践：GoFから現代的なDI実装まで"
emoji: "🍣"
...
コンボボックスなどで `A`、`B`、`C` のような選択肢を選ぶUIにおいて、選択肢ごとに異なる処理を実装する状況を想定します。  
...
以後、本記事ではこの現代的な実装を`StrategyContext`と呼称することにします。  
同じ構造を`Resolver`・`Factory`と呼ぶ実装も多く、命名に業界標準はありません。
...
// ステップ3との対比：DI解決のためのプロパティが不要
// ✅ 振る舞いの契約だけに絞った純粋なインターフェース
public interface IStrategy
{
    void Execute();
}
...
これを隠蔽するためにFactoryを実装する必要がありますが、結果として問題を解決するために問題を増やすという複雑な構造になります。  
...
> **💡 インターフェースの純粋さという観点では一つ利点がある**
>
> ステップ3では`IStrategy`に「DI解決のための識別子」として`SupportedOption`プロパティを持たせる必要がありましたが、これはストラテジーの「振る舞いの契約」ではありません。
> Keyed Services方式ではその責務をDI登録側に委ねられるため、インターフェースをより純粋に保てます。
> ただしこれは設計上の美しさの話であり、動的選択での採用を覆すほどの優位性はありません。

### Keyed Servicesの本来の価値 (静的選択と[FromKeyedServices])

Keyed Servicesが真価を発揮するのは「このサービスには常にこの実装」という静的選択の場面です。  
コンパイル時点でどの戦略を使うかが決まっている場合、`[FromKeyedServices]`属性で直接注入でき、FactoryもStrategyContextも不要です。
...
これにより`ObjectDisposedException`、スレッドセーフでないDbContextの共有によるデータ破壊、メモリリークが発生します。  
開発時は単一リクエストでしかテストしないため気づきにくいのが厄介なところです。
