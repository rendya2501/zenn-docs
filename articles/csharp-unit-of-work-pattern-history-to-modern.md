...
数年前、SQLを書く場所もトランザクション境界も曖昧なオレオレフレームワークを使用したプロジェクトに参加して炎上を経験しました。  
「どうすればもっとマシになるのか？」と考えて勉強していたら、リポジトリパターンやUnit of Workパターンにたどり着きましたが、当時は満足に勉強する時間が取れず、積み残し状態になってしまいました。  
ようやく時間が出来たのと、AIが発達したのもあって、「今こそ積み残した案件をしっかり理解したい。とりあえず、Unit of Workパターンから始めよう」と思い、サンプルプロジェクトを作りつつ、ドキュメントをまとめていたら、面白くなってそれなりの量になったので、公開することにしました。  
